...
struct _ThreadLocal {
    static var value: UnsafeMutableRawPointer? {
        get {
            return Thread.current.threadDictionary["TrailingEdgeObservation"] as? UnsafeMutableRawPointer
        }
        set {
            Thread.current.threadDictionary["TrailingEdgeObservation"] = newValue
        }
    }
}
