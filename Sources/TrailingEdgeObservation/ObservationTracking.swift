...
//@_spi(SwiftUI)
internal struct ObservationTracking: Sendable {
  enum Id {
    case willSet(Int)
    case didSet(Int)
    case full(Int, Int)
  ...
  struct Entry: @unchecked Sendable {
    let context: TrailingEdgeObservationRegistrar.Context

    var properties: Set<AnyKeyPath>
    
    init(_ context: TrailingEdgeObservationRegistrar.Context, properties: Set<AnyKeyPath> = []) {
...
//  @_spi(SwiftUI)
  internal struct _AccessList: Sendable {
    internal var entries = [ObjectIdentifier : Entry]()

    internal init() { }
    
    internal mutating func addAccess<Subject: TrailingEdgeObservable>(
      keyPath: PartialKeyPath<Subject>,
      context: TrailingEdgeObservationRegistrar.Context
    ) {
...
//  @_spi(SwiftUI)
  internal static func _installTracking(
...
//@available(SwiftStdlib 5.9, *)
internal func generateAccessList<T>(_ apply: () -> T) -> (T, ObservationTracking._AccessList?) {
...
//@available(SwiftStdlib 5.9, *)
internal func withObservationTracking<T>(
  _ apply: () -> T,
...
public func withDidSetObservationTracking<T>(
    _ apply: @escaping () -> T
) {
    _ = withObservationTracking(apply, didSet: { tracking in
        tracking.cancel()
        withDidSetObservationTracking(apply)
    })
}
