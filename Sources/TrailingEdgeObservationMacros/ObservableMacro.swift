...
public struct TrailingEdgeObservableMacro {
  static let moduleName = "TrailingEdgeObservation"

  static let conformanceName = "TrailingEdgeObservable"
  ...
  static let registrarTypeName = "TrailingEdgeObservationRegistrar"
  static var qualifiedRegistrarTypeName: String {
    return "\(moduleName).\(registrarTypeName)"
  }

  static let trackedMacroName = "TrailingEdgeObservationTracked"
  static let ignoredMacroName = "TrailingEdgeObservationIgnored"
...
extension DiagnosticsError {
  init<S: SyntaxProtocol>(syntax: S, message: String, domain: String = "TrailingEdgeObservation", id: ObservationDiagnostic.ID, severity: SwiftDiagnostics.DiagnosticSeverity = .error) {
...
extension TrailingEdgeObservableMacro: MemberMacro {
  ...
  >(
    ...
    if declaration.isEnum {
      ...
      throw DiagnosticsError(syntax: node, message: "'@TrailingEdgeObservable' cannot be applied to enumeration type '\(observableType.text)'", id: .invalidApplication)
    }
    if declaration.isStruct {
      // structs are not yet supported; copying/mutation semantics tbd
      throw DiagnosticsError(syntax: node, message: "'@TrailingEdgeObservable' cannot be applied to struct type '\(observableType.text)'", id: .invalidApplication)
    }
    if declaration.isActor {
      // actors cannot yet be supported for their isolation
      throw DiagnosticsError(syntax: node, message: "'@TrailingEdgeObservable' cannot be applied to actor type '\(observableType.text)'", id: .invalidApplication)
    }

    var declarations = [DeclSyntax]()

    declaration.addIfNeeded(TrailingEdgeObservableMacro.registrarVariable(observableType), to: &declarations)
    declaration.addIfNeeded(TrailingEdgeObservableMacro.accessFunction(observableType), to: &declarations)
    declaration.addIfNeeded(TrailingEdgeObservableMacro.withMutationFunction(observableType), to: &declarations)

    return declarations
  }
}

extension TrailingEdgeObservableMacro: MemberAttributeMacro {
  public static func expansion<
    Declaration: DeclGroupSyntax,
  ...
  >(
    ...
    if property.hasMacroApplication(TrailingEdgeObservableMacro.ignoredMacroName) ||
       property.hasMacroApplication(TrailingEdgeObservableMacro.trackedMacroName) {
      return []
    }


    return [
      AttributeSyntax(attributeName: IdentifierTypeSyntax(name: .identifier(TrailingEdgeObservableMacro.trackedMacroName)))
    ]
  }
}

extension TrailingEdgeObservableMacro: ExtensionMacro {
  public static func expansion(
    of node: AttributeSyntax,
...
public struct TrailingEdgeObservationTrackedMacro: AccessorMacro {
  public static func expansion<
    Context: MacroExpansionContext,
...
extension TrailingEdgeObservationTrackedMacro: PeerMacro {
  public static func expansion<
    Context: MacroExpansionContext,
    Declaration: DeclSyntaxProtocol
  >(
    ...
    let storage = DeclSyntax(property.privatePrefixed("_", addingAttribute: TrailingEdgeObservableMacro.ignoredAttribute))
    return [storage]
  }
}

public struct TrailingEdgeObservationIgnoredMacro: AccessorMacro {
  public static func expansion<
    Context: MacroExpansionContext,
    Declaration: DeclSyntaxProtocol
  >(
